...
$today_tasks = $conn->query ("SELECT * from tasks where user_id = $user_id AND date = '$today'");
$yesterday_tasks = $conn->query("SELECT * from tasks where user_Id =  $user_id and date = '$yesterday'");
        ...
        <div class="tabs-container">
            ...
            <div class="tab-content">
                <div class="flex justify-between items-center border-b pb-5 border-black/50">
                    ...
                    <form method="POST" action="./backend/auth/logout.php" class="flex">
                        <button type="submit" class="prim-btn">Clock Out</button>
                    </form>
                </div>
                        ...
                        <div class="bg-blue-950 text-white w-1/2 min-h-52 rounded-2xl p-10">
                            ...
                            <form class="ml-3">
                                <ol class="list-decimal list-inside">
                                    <?php 
                                   while ($row = $yesterday_tasks->fetch_assoc()):
                                  ?>
                                    <li>
                                        <input type="checkbox" name="completed_task[]" value="<?php echo $row['id'] ?>"
                                            <?php echo $row['completed'] ? 'checked disabled' : '' ?> />

                                        <span
                                            class="<?php echo $row['completed'] ? 'line-through text-gray-500' : ""; ?>">
                                            <?php echo htmlspecialchars($row['task']); ?> </span>
                                    </li>
                                    <?php endwhile; ?>
                                    <!-- <li>lkdlk</li>
                                    <li>lkdlk</li> -->
                                </ol>
                                <button type='submit' class='prim-btn'>Mark as Completed</button>
                            </form>
                        </div>
                        <div class="bg-blue-950 text-white w-1/2 min-h-52 rounded-2xl p-10">
                            ...
                            <form class="ml-3">
                                <ol class="list-decimal list-inside">
                                    <?php 
                                   while ($row = $today_tasks->fetch_assoc()):
                                  ?>
                                    <li>
                                        <input type="checkbox" name="completed_task[]" value="<?php echo $row['id'] ?>"
                                            <?php echo $row['completed'] ? 'checked disabled' : '' ?> />
